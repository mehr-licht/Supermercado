...
//vec teria apenas dois elementos, que serï¿½o iguais, mas um representa a origem e outro o destino
	...
	} else if (supermarket.getID() == sourceID
			|| supermarket.getID() == destID) { //se a source ou o destino sï¿½o o parque
...
//vec teria o caminho mais perto de source a dest
		...
		part2 = myGraph.getPath(getNodeByID(supermarket.getID()),
				getNodeByID(destID)); //caminho mais cuto do parque ao dest
...
void map::assignSupers() {
	for (unsigned int i = 0; i < this->vecSupermarket.size(); i++) {
